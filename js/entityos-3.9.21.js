...
entityos.init = function (data)
{
	$.ajaxSetup(
	{
		...
		beforeSend: function (oRequest)
		{
			...
			oRequest.setRequestHeader("auth-sid", entityos._scope.session.sid);
			oRequest.setRequestHeader("auth-logonkey", entityos._scope.session.logonkey);
		}
	});
...
entityos.invoke = function (param)
{
	...
	if (param.method == undefined & param.url == undefined)
	{
	...
	else
	{
        if (param.url == undefined)
        {
            ...
            param.url = '/rpc/' + endpoint + '/?method=' + (param.method).toUpperCase();
        }
...
entityos.retrieve = function (param)
{
	...
	else
	{
		...
		else
		{
            ...
            if (_.has(entityos, '_scope.app.options.forceMore'))
            {
                if (entityos._scope.app.options.forceMore == true)
                {
                    param.url = param.url + '&forcemore=Y'
                }
            }
									...
									if (data.exists)
									{
                                        ...
                                        */
										...
										if (split !== undefined)
										{
											if (data.value != undefined)
											{	
												data.values = data.value.split(split);
					...
					if (!sameAsLast)
					{
						...
						{	
							if (url.substr(0,1) == '/')
							{
								...
								{
									url = 'https://api.entityos.cloud' + url;
								}
							}
							...
							if (param.datareturn != undefined)
							{
								if (data == undefined) {data = {}}

								if (data.datareturn == undefined)
								{
									data.datareturn = param.datareturn;
								}
								else
								{
									data.datareturn = data.datareturn + ',' + param.datareturn;
								}
					...
					queue:
							{
											...
											for (var key in data)
									  		{
									     		if (data.hasOwnProperty(key))
									     		{
													...
													*/
													...
													if (s != undefined && entityos._scope.app.options.unescapeHTML)
									     			{
								...
								render: function (selector, param, data, template)
								{
									...
									if (_.isArray(selector)) {selector = _.join(selector, '')}
									...
									else
									{
										...
										if (_.isArray(appendSelector)) {appendSelector = _.join(appendSelector, '')}
										if (_.isArray(disableSelector)) {disableSelector = _.join(disableSelector, '')}
										if (_.isArray(enableSelector)) {enableSelector = _.join(enableSelector, '')}
										if (_.isArray(hideSelector)) {hideSelector = _.join(appendhideSelectorSelector, '')}
										if (_.isArray(showSelector)) {showSelector = _.join(showSelector, '')}
					...
					dropzone: 
					{
						...
						init: function (param)
						{
							...
							if (message == undefined)
							{
                                ...
                                if (!autoUpload)
                                {
                                    message = message + '</br><span class="text-muted">(Then click Upload)</span>';
                                }
							}
							...
							var publicType = entityos._util.param.get(param, 'publicType').value;
							var title = entityos._util.param.get(param, 'title').value;
							...
							if (true)
							{
								...
								entityos._util.attachment.dropzone.object[name].on('sending', function(file, xhr, formData)
								{
									...
									if (data != undefined)
									{
                                        ...
                                        if (data.title != undefined) {formData.append('title0', data.title)}
									}
								});
